...
void aes_subBytes(ap_uint<8> buf[16]
, ap_uint<8> sbox[256]
) {
...
void aes_addRoundKey(ap_uint<8> buf[16]
, ap_uint<8> key[32]
, ap_uint<1> offset) {
...
void aes_addRoundKey_cpy(ap_uint<8> buf[16]
, ap_uint<8> key[32]
, ap_uint<8> cpk[32]
) {
...
void aes_mixColumns(ap_uint<8> buf[16]
  ...
  for(int i = 0; i < 4; i++) {
    ...
    ap_uint<8> e = (a ^ (b ^ (c ^ d)));
    //---
    buf[(4 * i)] = (a ^ (e ^ rj_xtime((a ^ b))));
    //---
    buf[((4 * i) + 1)] = (b ^ (e ^ rj_xtime((b ^ c))));
    //---
    buf[((4 * i) + 2)] = (c ^ (e ^ rj_xtime((c ^ d))));
    //---
    buf[((4 * i) + 3)] = (d ^ (e ^ rj_xtime((d ^ a))));
  }
}
void aes_expandEncKey(ap_uint<8> k[32]
, ap_uint<8> rc, ap_uint<8> sbox[256]
) {
