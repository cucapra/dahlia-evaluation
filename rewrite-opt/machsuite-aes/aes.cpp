...
void aes_addRoundKey_cpy(ap_uint<8> buf[16], ap_uint<8> key[32], ap_uint<8> cpk[32]) {
  ...
  #pragma HLS ARRAY_PARTITION variable=cpk cyclic factor=4 dim=1
...
void aes_mixColumns(ap_uint<8> buf[16]) {
  ...
  for(int i = 0; i < 4; i++) {
    ...
    ap_uint<8> a = buf[((i + ((0 >> 0) * 4)) + (0 & 0))];
    
    //---
    ap_uint<8> b = buf[((i + ((1 >> 0) * 4)) + (1 & 0))];
    
    //---
    ap_uint<8> c = buf[((i + ((2 >> 0) * 4)) + (2 & 0))];
    
    //---
    ap_uint<8> d = buf[((i + ((3 >> 0) * 4)) + (3 & 0))];
    ...
    buf[((i + ((0 >> 0) * 4)) + (0 & 0))] = (a ^ (e ^ temp));
    rj_xtime((a ^ b));
    //---
    buf[((i + ((1 >> 0) * 4)) + (1 & 0))] = (b ^ (e ^ temp));
    rj_xtime((b ^ c));
    //---
    buf[((i + ((2 >> 0) * 4)) + (2 & 0))] = (c ^ (e ^ temp));
    rj_xtime((c ^ d));
    //---
    buf[((i + ((3 >> 0) * 4)) + (3 & 0))] = (d ^ (e ^ temp));
    rj_xtime((d ^ a));
  }
}
...
void aes(ap_uint<8> key[32], ap_uint<8> enckey[32], ap_uint<8> deckey[32], ap_uint<8> k[32], ap_uint<8> buf[16]) {
  ...
  #pragma HLS ARRAY_PARTITION variable=enckey cyclic factor=4 dim=1
  
  #pragma HLS ARRAY_PARTITION variable=deckey cyclic factor=4 dim=1
