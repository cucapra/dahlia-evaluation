...
#pragma HLS INTERFACE s_axilite port=position_z
  ...
  int maxx =0; int minx = 0; int numberx = 0; int totalx = 0;
  int maxp =0; int minp = 0; int numberp = 0; int totalp = 0;
  int maxq =0; int minq = 0; int numberq = 0; int totalq = 0;
  ...
  loop_grid1_z: for( b1.z=MAX(0,b0.z-1); b1.z<MIN(blockSide,b0.z+2); b1.z++ ) {
    ...
    loop_p: for( p_idx=0; p_idx<n_points[b0.x][b0.y][b0.z]; p_idx++ ) {
      ...
      force_local[b0.x][b0.y][b0.z][p_idx].z = sum_z ;
        ...
        printf("x max: %d, min: %d, total: %d, number: %d, avg: %d \n",maxx, minx, totalx, numberx, totalx/numberx);
        printf("p max: %d, min: %d, total: %d, number: %d, avg: %d \n",maxp, minp, totalp, numberp, totalp/numberp);
        printf("q max: %d, min: %d, total: %d, number: %d, avg: %d \n",maxq, minq, totalq, numberq, totalq/numberq);
