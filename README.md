...
> We recommend reviewers use as many physical cores as they have available to
> speed up this section. The script uses [GNU `parallel`][parallel] to speed up
> execution. Actual runtime will depend on the number of cores available.
...
The script generates files with the names `*-accepted` in the repository root
...
the reviewers with access to our deployment. Since it is expensive to keep the
...
4. Wait for all jobs to enter the `done` phase. Once this happens, exit the
   watch script. If a job is in `failed` state, see the instructions below.
...
same configuration and ended up in an erroneous state.

To re-run a failed job:

1. Copy the reported job and open the [Polyphemus deployment](cerberus.cs.cornell.edu:5000).
2. Ctrl-F search the job ID and click on the link.
3. On the job page, click on the "state" dropdown, select "Start Make", and click on "set".
4. The job will then be restarted.
5. **Please message us** if any of the jobs are reported as `failed` if this
...
**Note that the Polyphemus deployment for PLDI AEC reviewers does not support FPGA machines.**
...
Because of the amount of direct interaction required, we assume that the reader
has read the documentation for [Polyphemus] and understands the basics of
instance and jobs folder.
...
Follow the workflow from the "Sensitivity Analysis" study above to upload and
run the jobs.
